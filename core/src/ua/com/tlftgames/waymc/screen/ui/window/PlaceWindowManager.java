...
public class PlaceWindowManager extends ActionWindowManager {
    ...
    public PlaceWindowManager(UIGroup group) {
        super(group, "place");
        needTutorial = Settings.getInstance().getTutorialEnable() && !Tutorial.isTutorialShowed(Tutorial.TUTORIAL_CRIME);
    }
    ...
    public void startSearch() {
        ...
        if ((place.getIndex() == needSearchPlace || place.getType() == needSearchPlace)
            ...
            if (GameCore.getInstance().getPlaceManager().isSearchIndexInArray(
                    searchIndex, Config.getInstance().searchNeedMoneyIndexes)) {
                this.startSearchQuest(SEARCH_QUEST_TYPE_MONEY);
            } else if (GameCore.getInstance().getPlaceManager().isSearchIndexInArray(
                    searchIndex, Config.getInstance().searchNeedItemIndexes)) {
    ...
    private void showSearchQuestChoices(final int quest_type) {
        ...
        TextButton acceptBtn = this.getHelper().createTextButton(
                GameCore.getInstance().getPlaceManager().getCurrentSearchText() + ".variant.accept");
        switch (quest_type) {
            case SEARCH_QUEST_TYPE_MONEY:
            ...
            case SEARCH_QUEST_TYPE_ITEM:
                acceptBtn.addListener(new ClickListener() {
                    @Override
                    public void clicked(InputEvent event, float x, float y) {
                        if (!GameCore.getInstance().getItemManager().canCreateItem(Config.getInstance().searchNeedItemName)) {
                            GameCore.getInstance().getNotificationManager()
        ...
        TextButton cancelBtn = this.getHelper().createTextButton(
                GameCore.getInstance().getPlaceManager().getCurrentSearchText() + ".variant.cancel");
                    ...
                    GameCore.getInstance().getPlaceManager().getCurrentSearchText(), vars, new ClickListener() {
                        ...
                        public void clicked(InputEvent event, float x, float y) {
                            if (GameCore.getInstance().getPlaceManager().isSearchIndexInArray(
                                    GameCore.getInstance().getPlaceManager().getCurrentSearchPlaceIndex(),
                                    Config.getInstance().searchNeedItemIndexes)) {
                                GameCore.getInstance().getItemManager().addOwnReceipt(Config.getInstance().searchNeedItemName);
                            }
                            PlaceWindowManager.this.getUIGroup().getCurrentTypeWindowManager()
                                    .startMandatoryQuest();
                        }
                    });
        } else {
            //TODO: change to search fail image
            this.getWindow().setPlaceImageTexture();
            PlaceWindowManager.this.showActionResult(
